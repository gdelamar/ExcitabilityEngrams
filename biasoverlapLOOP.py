...
np.disp('Starting bias overlap on 100 seeds and 3 different current2')
...
freezing2 = np.zeros((Nrep,4*Ncurrent2))
freezing2bis = np.zeros((Nrep,4*Ncurrent2))
current1 = 0
loop = 0
...
for seed in range(0,Nrep):
	...
	for current2set in L_current2:
		np.disp("Seed = "+str(seed)+", current2set = "+str(current2set))
		...
		for current2 in [0,current2set]:
			for delay in [6*3600 *1000,24*3600 *1000]:
				...
				np.disp([current2, delay/3600/1000,freezing2[loop,cc],freezing2bis[loop,cc]])		
				cc+=1
	loop +=1
    
    
with open('freezing2_biasoverlapLOOP_10_06.pkl', 'wb') as f:
	pickle.dump([freezing2], f)
with open('freezing2bis_biasoverlapLOOP_10_06.pkl', 'wb') as f:
